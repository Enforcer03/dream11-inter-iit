...
def optimize_team_sharpe(
    ...
    return selected_players, pd.DataFrame(
        {"player": stats_df["player"], "weight": w_values}
    )


def optimize_team_advanced(
    stats_df: pd.DataFrame,
    ...
    consistency = np.nan_to_num(
        consistency, nan=0.0, posinf=np.nanmax(consistency[~np.isinf(consistency)])
    )
    ...
    if len(valid_consistency) > 0:
        ...
        prob += (
            pulp.lpSum([consistency[i] * x[i] for i in players])
            >= mean_consistency * num_players / 2
        )

    if entropy_score > 0:
        prob += (
            pulp.lpSum([entropy_score * x[i] for i in players])
            >= entropy_score * num_players / 2
        )
    ...
    for team in stats_df["team"].unique():
        team_players_indices = [
            i for i, p in enumerate(players) if stats_df.iloc[i]["team"] == team
        ]
    ...
    return selected_players, pd.DataFrame(
        {
            ...
            "weight": [
                x[i].value() if i in selected_indices else 0
